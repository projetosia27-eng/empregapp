...
export class ResumeReaderController {
  static async parseResume(req: any, res: any) {
    try {
      const { pdfBase64, parserMethod, extractedText } = req.body;
      ...
      // 2. Tentar extrair o texto limpo do PDF se não tiver sido enviado do cliente (ou se for muito curto)
      let textExtra = '';
      ...
      const forceFree = parserMethod === 'free' || !apiKey;

      if (forceFree) {
        console.log(`[ResumeReaderController] Processando preenchimento em modo Gratuito Sem IA (Heurística Local) - Motivo: ${parserMethod === 'free' ? 'Solicitado pelo usuário' : 'Chave API Indisponível'}`);
